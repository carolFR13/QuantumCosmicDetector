...
struct GeometryPoint {
    double x, y, z;
    GeometryPoint() : x(0), y(0), z(0) {}  // Add default constructor
    GeometryPoint(double x_, double y_, double z_) : x(x_), y(y_), z(z_) {}
};
...
TTree* filter_first_hits(TTree* tree) {
    ...
    for (const auto& pair : bestHitIndex) {
        ...
        outVolName[127] = '\0';  // Ensure null-termination
        filtered->Fill();
    }

    TFile* tempFile = new TFile("filtered_data.root", "RECREATE");
    filtered->Write();
    tempFile->Close();
    
    std::cout << "Filtered tree saved to filtered_data.root" << std::endl;
...
void analysis_bars() {
    ...
    TFile* filteredFile = TFile::Open("filtered_data.root");
    ...
    TFile* outFile = TFile::Open("reconstructed_data.root", "RECREATE");
    if (outFile && !outFile->IsZombie()) {
        reco_tree->Write();
        outFile->Close();
        std::cout << "Output saved to reconstructed_data.root" << std::endl;
    } else {
    ...
    TFile* recoFile = TFile::Open("reconstructed_data.root");
